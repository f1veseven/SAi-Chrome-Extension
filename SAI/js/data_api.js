...
async function getHacking8Data(page){
    ...
    try{
        await fetch(`https://i.hacking8.com/index/index?page=${page}`)
        .then(async res => {
            ...
            if(trDom.length){
                for(let item of trDom){
                    ...
                    let i = genareteMData(originPath + newhref, aDom.innerHTML, author.innerHTML, summary ? summary.innerHTML : '');
                    dataList.push(i);
                }
            }
            olist.data = dataList;
